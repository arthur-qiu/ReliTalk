    ...
    def __getitem__(self, idx):
        ...
        if not self.only_json:
            ground_truth = {
                ...
                "normal": torch.from_numpy(rend_util.load_rgb(self.data["normal_paths"][idx], self.img_res).reshape(3, -1)
                        .transpose(1, 0)).float() * sample["object_mask"].unsqueeze(1).float()
                        + (1 - sample["object_mask"].unsqueeze(1).float()),
                "bg_img": torch.from_numpy(rend_util.load_rgb(self.data["image_paths"][idx], self.img_res)).reshape(3, -1)
                        .transpose(1, 0).float(),
            }
        else:
            ground_truth = {}
        ...
        if self.use_semantics:
            if not self.only_json:
                ...
                # face_mask = torch.sum(ground_truth["semantics"][:,:-2], 1)
                # ground_truth["normal"] = torch.from_numpy(rend_util.load_rgb(self.data["normal_paths"][idx], self.img_res).reshape(3, -1)
                # .transpose(1, 0)).float() * face_mask.unsqueeze(1) * sample["object_mask"].unsqueeze(1).float() + (1 - face_mask.unsqueeze(1) * sample["object_mask"].unsqueeze(1).float())
